...
export const Card = ({
  ...
  return (
    <div className="flex flex-col justify-center items-center shadow outline-4 border">
      <Link href={`/products/${slug}`}>
        <div className="px-4 border-gray-500 rounded border-[2px] py-1 ">
          <div>
            <svg
              width="9"
              height="9"
          ...
          <div>
            <div className="flex flex-wrap items-center gap-1 pt-1 tablet:gap-2">
              <span
                className="zui-tag zui-tag__alert inline-flex items-center whitespace-nowrap rounded-lg border-[0.5px] border-red-600 bg-white h-[18px] gap-1 px-1"
              ...
              <div
                data-test-id="nonSpecialPriceValue"
                className="text-grey-60 line-through text-base"
              >
                Php&nbsp;9,469
              </div>
              <div className="text-base font-bold lowercase ">-58%</div>
            </div>
          </div>
        </div>
      </Link>

      <div className="flex gap-4 rounded border-spacing-x-7  outline outline-offset-2 outline-1 ... divide-y-0 m-2 ring-4 justify-between px-5 py-0">
        <Button name={`-`} onClick={handleDislikeCount}></Button>
        {likeCount}
        <Button name={`+`} onClick={handleLikeCount}></Button>
      </div>
      <div className="flex items-center outline-double ... ring-2 ring-inset border-solid hover:border-dotted font-serif m-3 rounded px-20 py-1 ">
        <Button
          className="flex text-center hover:scale-110 duration-75 ease-in "
          name={`Add to Cart`}
          onClick={() => {}}
        />
      </div>
